...
- **Sector Performance** (`/sector`) — SPX shown without `$` prefix (index); sector ETFs with `$`. Page header: title + all-caps subtitle inline (matching vol pages), EOD/LIVE buttons right-aligned with date tag on EOD.
