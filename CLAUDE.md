...
- **`d_extended`** = `D > B + 0.5·abs(B−A)` (50%-of-AB threshold; resets when a new C forms). Break level = `max(B, C)` uptrend / `min(B, C)` downtrend while True (C may walk past B). Drives break level selection in warn flags, the popup `*`, and the break machine; independent of `structural_state`. Distinct from the daily-overshoot `lrr_extended`/`hrr_extended` flags — three separate "extended" concepts, never conflate.
...
- **Sidebar** (`Sidebar.js`) — must stay `position: fixed` (sticky re-introduces Recharts `ResponsiveContainer` ResizeObserver stutter). Add dashboards by appending to `NAV_ITEMS` — no other files change. Admin is NOT in the sidebar (direct URL only). **VOL** is a collapsible parent with five children: SPX VOL (`/vol`), MACRO VOL (`/vol/macro`), BOND VOL (`/vol/bond`), HY CREDIT (`/vol/hy-credit`), MKT VOLUME (`/vol/market-volume`). Clicking VOL toggles the submenu open/closed (no navigation); only child items navigate. Collapsed sidebar: clicking VOL icon navigates to SPX VOL (default). Sub-items auto-expand when on any `/vol/*` route. See **ADR-011**.
...
- **MKT VOLUME dashboard** (`/vol/market-volume`) — NYSE market volume & breadth. Top table: NYSE Total Volume only (TVOL) with % vs Prior Day / 1M / 3M / 1Y averages. Cumulative A/D line chart (running sum of daily $ADD) with 1Y/2Y/MAX toggle. Breadth stats table: Advancing Issues (ADVN), Declining Issues (DECN), Net A/D, A/D Ratio. Data from `GET /api/vol/market-volume` (`backend/routers/vol.py`). Schwab-only (`$`-symbols via `get_price_history()`); blank in dev (no token). EOD date from `volume_date` (last date in history_json), not `updated_at`. Up Volume (UVOL) and Down Volume (DVOL) rows removed from the table (kept in backend for potential future use). Net Volume row removed from backend (UVOL−DVOL oscillates around zero, making % comparisons nonsensical). Up/Down Volume chart removed.
- **Vol index intraday quotes** (`schwab_market_data.py`) — `schwab_fetch_intraday_quotes` calls individual `get_quote()` for 6 vol indices (VIX→`$VIX`, VXN→`$VXN`, RVX→`$RVX`, GVZ→`$GVZ`, OVX→`$OVX`, MOVE→`$MOVE`) after the batch quote loop. Schwab batch `get_quotes()` silently drops `$`-symbol indices; individual `get_quote()` works. Updates `price_cache.close` + `updated_at` only (no history/cache_date). Breadth symbols (`$TVOL/$UVOL/$DVOL/$ADD`) return garbage from `get_quote()` — excluded; remain 1-day lagged via EOD history fetch.
...
│   │   │   ├── MarketVolumeChart.js       ← /vol/market-volume — NYSE total volume table + cumulative A/D line + breadth stats
...
│   │   └── analyst_data.py               ← On-demand analyst momentum data (yfinance upgrades_downgrades + price targets); individual stocks only
│   └── routers/
│       ├── market_data.py
...
│       ├── security.py                    ← GET /api/security/{ticker}/detail — aggregated deep-dive; GET /{ticker}/summary — async on-demand AI summary; POST /{ticker}/summary — admin regen (ADR-030); GET /{ticker}/analyst — on-demand analyst momentum
│       ├── macro.py                       ← GET /api/macro/correlations — rolling Pearson correlations vs DXY + 52-wk rolling stats
│       └── debug.py                       ← GET /api/debug/pivots — pivot engine trail viewer with break persistence layer + diagnostics
...
Range: 0–100 (v2.2 additive formula; NATH/warn baked into structural pillar)
  Base max:  Structural 50 (45+5 NATH) + Quad 20 + Volume 15 + VIX 15 = 100
...
**Break level = C normally; `max(B, C)` uptrend / `min(B, C)` downtrend when `d_extended = True`.** When C walks above B (uptrend) or below B (downtrend), the break level rises/falls to C — the higher/lower of the two is always the active break level. The break level applies to all state transitions (BREAK_OF_TRADE, BREAK_OF_TREND, BREAK_CONFIRMED) and to all warn flags (⚠ on LRR/HRR cells).
...
**Critical rules:**
- **Break level = C normally; `max(B, C)` uptrend / `min(B, C)` downtrend when d_extended = True** — when C walks past B, C becomes the break level; applies to BREAK_OF_TRADE, BREAK_CONFIRMED, and warn flags
...
| `d_extended=True` | **max(B,C)** uptrend / **min(B,C)** downtrend — C may walk past B | **D** — extended high/low; "can BB target still reach the peak?" |
...
53. **Three independent "extended" concepts — never conflate:**
    - `d_extended` (Boolean field) — D > B + abs(B-C); break level = `max(B,C)` uptrend / `min(B,C)` downtrend; drives warn flags and popup `*`; NOT in structural_state
...
60. **`d_extended` + direction determine break level** — when `d_extended=True`, break level = `max(B, C)` uptrend / `min(B, C)` downtrend (C may walk past B). `is_warning`, `_compute_warn_flags`, popup `tradeBreakIsB`/`trendBreakIsB`, and `warnTip` all read `d_extended` + direction; `_breakIsB` helper in App.js returns false when C has walked past B. Never derive from state string comparison
...
119. **Volume pillar text says "volume signals" not "OBV layers"** — pillar detail text in `SecurityAnalysis.js` uses "volume signals aligned" (full direction) and "one volume signal directional" (leaning direction). The OBV implementation detail is hidden from the user-facing label.
...
131. **Break persistence layer (`signals.py` + `debug.py`)** — the pivot engine is stateless (recomputes from scratch each bar). When the engine loses a held structure (stale C, A aging out, both-broken, finds a different structure), the persistence layer checks the prior stored state's break level before transitioning. **VALID→engine disagrees:** if price crossed the break level → BREAK_OF_TRADE/BREAK_OF_TREND; if price is still on the correct side → hold the prior VALID state + pivots. **BREAK→next bar:** if price still broken → BREAK_CONFIRMED; if recovered → let engine result through. Applies to Trade, Trend, and LT timeframes. Break level = C normally; B when `d_extended=True`. The `engine_produced_break` exemption (which let unrelated broken structures override valid held state) was removed. Debug tool captures `engine_state` + `engine_pivots` + `engine_direction` for visibility into what the engine actually computed vs what the persistence layer conveyed. Diagnostics panel (`_diagnose_no_structure`) fires on NO_STRUCTURE bars showing raw/walked ABCs, intact checks, stale C, and reason codes.
132. **Lookback buffer for A cutoff (`pivot_engine.py`)** — `_MAX_A_LOOKBACK` cutoff extended backward by `bar_window` so pivots near the edge of the logical lookback have enough bars on both sides to confirm. Effective lookbacks: Trade=95 (90+5), Trend=210 (200+10).
133. **D requirement for new structures (OPEN DESIGN DECISION — DO NOT IMPLEMENT)** — the engine currently returns UPTREND_VALID/DOWNTREND_VALID for new ABC structures without D established (price hasn't closed beyond B). This is unreliable — AAPL 2/25 showed the same structure (A=246.70, B=278.12, C=255.78, no D) returned UPTREND_VALID and immediately failed; it only succeeded on 5/5 when D finally established. The D requirement should be enforced for NEW structures but relaxed for REPAIRS of former trends (where D had previously established, proving the thesis). The filter: only relax D when A postdates the prior break — timestamps the structure as a product of the new regime. Related open decisions: A-walking mechanism, extending lookback approaches. **Do not implement until design is finalized.**

134. **C > B break level when d_extended (`pivot_engine.py`, `conviction_engine.py`, `signals.py`, `debug.py`, `intraday_monitor.py`, `App.js`, `SecurityAnalysis.js`)** — when `d_extended=True` and C has walked past B (uptrend: C > B; downtrend: C < B), the break level uses the higher/lower of the two: `max(B, C)` uptrend / `min(B, C)` downtrend. Previously `d_extended` always used B, but C-walking can advance C above B — leaving the break level at a stale B far below the current structure. All 7 backend locations (`_price_on_correct_side`, `compute_d_and_state`, `is_warning`, `_compute_warn_flags`, Trend Level, persistence layer ×2, debug break_level, intraday BREAK_OF_TRADE) and 2 frontend locations (popup `_breakIsB` helper, SecurityAnalysis warn tooltips) updated. AAPL Trend Feb 2026: C=266.25 walked above B=239.78 → break level correctly rose to 266.25 (was 239.78). Never revert to unconditional `break_level = B` when d_extended.

130. **Analyst Momentum Signal (`analyst_data.py`) — on-demand, not cached** — `GET /api/security/{ticker}/analyst` fetches yfinance `upgrades_downgrades` + `analyst_price_targets` live (not scheduled, not DB-cached). Returns `{available: false}` for ETFs/indices/futures/FX (no analyst coverage). Individual stocks (~9 of ~51 tickers) return: rating momentum (net upgrades over 30D/90D/180D), price target momentum (PT raises/lowers over 90D), current consensus (buy/hold/sell distribution, buy%), peak consensus flag (buy% ≥ 90%), days since last upgrade, recent actions (last 10), and a composite momentum signal (Strong Upgrade / Upgrade / Neutral / Moderate Downgrade / Downgrade / Peak Consensus). Grade classification via `BUY_GRADES`/`SELL_GRADES` frozensets in `analyst_data.py`. PT-only promotion requires non-negative `net_90d` (prevents false Upgrade when downgrades dominate). UI: ANALYSIS tab on Security Analysis page — momentum banner (color-coded), price target visual bar (flex row: `$low [dot—bar—dot] $high`; labels flank bar; close hash+label on bar when in range, else text outside; card padding 25px), consensus stacked bar (hold = amber), rating momentum table, recent actions table. Top section: ANALYST MOMENTUM + PT UPSIDE fields next to Volatility Regime (fontWeight 700, colored, no MONO).
