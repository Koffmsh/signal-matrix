...
Volume (0 / +10 / +15):
  ...
  obv_confirming: both MA20 slope AND 21-bar lookback confirm consensus direction
  Consensus direction: Trade OR Trend (whichever is directional; both aligned → use that;
    one Neutral → use the other; opposing → no consensus, volume_score = 0; both Neutral → 0)
...
Vol (0–15 — routed per vol index; flat +15 when no index applies):
  ...
  Per-index thresholds (investable_ceiling, danger_floor):
    VIX (19,30), VXN (22,32), RVX (22,32), GVZ (22,32), OVX (38,60), MOVE (85,120)
...
41. **OBV direction uses a 21-bar lookback** — `_obv_direction()` builds OBV, compares current OBV vs OBV 21 bars ago (`_OBV_LOOKBACK = 21`): higher → Bullish, lower → Bearish, equal → Neutral. Needs ≥ 22 OBV bars else Neutral. Supersedes the z-score regression method (ADR-017) — simpler, inherently self-scaling (same ticker vs itself), and no cross-ticker normalization needed. **Volume scoring uses consensus direction** (Trade OR Trend — whichever is directional; both aligned → use that; one Neutral → use the other; opposing → no consensus, volume_score = 0). **Two independent layers:** MA20 slope confirms consensus dir → +5; 21-bar lookback confirms consensus dir → +5. Both layers must confirm for `vol_signal = "Confirming"` (+10); both must oppose for `"Diverging"` (0); mixed = `"Neutral"` (+5 from whichever confirms). Acceleration bonus (+5, total 15) only when Confirming AND `obv_slope_trend` accelerating in consensus direction.
...
61. **Vol score tiers (v2.3 — asset-specific vol indices, direction-aware, Edgy eliminated)** — Each ticker routes to its relevant volatility index via `_resolve_vol_index(ticker, asset_class)`: ticker-level map (`VOL_INDEX_TICKER_MAP`) → Domestic Equities default to VIX → None (flat +15). **Vol indices:** VIX (broad SPX equities), VXN (Nasdaq-heavy: QQQ/NDX/XLK/SMH/SOXX/CIBR/QTUM/GRID/AAPL/MSFT/NVDA/AVGO/GOOGL/META/NFLX), RVX (Russell: IWM/RUT), GVZ (Gold: GLD/SGOL//GC), OVX (Oil: USO//CL/XOP/OIH), MOVE (Fixed Income: TLT//ZN/SHY/IEF/VGIT/LQD/MBB/PFF/TIP). **Per-index thresholds** `(investable_ceiling, danger_floor)` in `VOL_INDEX_THRESHOLDS`: VIX (19,30), VXN (22,32), RVX (22,32), GVZ (22,32), OVX (38,60), MOVE (85,120). **Three tiers (Edgy eliminated → folded into Choppy):** Bullish/Neutral: Investable+ (close < ceiling AND HRR < ceiling) +15 · Investable (close < ceiling) +10 · Choppy (ceiling–danger) +0 · Danger (≥ danger) +0. Bearish (asymmetric, +5 floor): Danger +15 · Choppy +10 · Investable +5. `get_vol_score(vol_close, vol_hrr, thresholds, viewpoint)` is the generic scorer; `get_vix_score()` wraps it with index routing. Security detail endpoint resolves vol index per ticker and returns `vol_index` field. Popup label: "Vol Regime" (not "VIX Regime"). **NATH Boost (×1.05):** unchanged — Viewpoint=Bullish AND trade HRR > `price_cache.ath`. Do not change without explicit instruction.
66. **Quad score is viewpoint-aware and probability-weighted (v2.0)** — `alignment = get_quad_alignment(asset_class, sector, current_quad)` → +1.0/0.0/-1.0. Viewpoint=Neutral → quad_score=0. **Aligned = (Bullish + Best) OR (Bearish + Worst)** — viewpoint must agree with the quad tailwind direction; Bearish + Best (or Bullish + Worst) = Misaligned. Aligned: +20 (prob≥0.45) or +15 (prob<0.45). Misaligned: -15 (prob≥0.45) or -11 (prob<0.45). Neutral alignment: 0. `quad_score` (Integer) is stored in `signal_output` and shown in popup (green/red/grey). `quad_mult` still written to `signal_output` for debug only — not in v2.0 formula and not shown in popup. Index sectors always return 0.
...
109. **Security Analysis pillar boxes show raw scores** — STRUCTURE: 0/25/50, VOLUME: 0/10/15, VOLATILITY: 0/5/10/15, QUAD: -15 to +20. Labels derived from raw values: STRUCTURE (Bullish/Bearish/Leaning Bullish/Leaning Bearish/Neutral — viewpoint-aware, see rule 111), VOLUME (Confirming+/Confirming/Neutral), QUAD (Aligned/Misaligned/Neutral). **VOLATILITY label uses `vix_regime` from backend** (not derived from score); "N/A" regime shows "Full Credit" (+15 flat, no applicable vol index). VOLATILITY detail text shows the resolved vol index name (VIX/VXN/RVX/GVZ/OVX/MOVE). Colors: STRUCTURE green≥50/amber≥25/grey, VOLUME green≥10/grey, VOLATILITY green≥10/grey (no amber — Edgy eliminated), QUAD green>0/red<0/grey.
...
112. **VRP regime is a separate labeled field** — the Security Analysis vol metrics row shows VRP as a percentage only; the regime label (Falling Knife / Gamma Trap / Yield Harvest / Upside Panic / Quiet Accumulation / Neutral) appears in its own "VOLATILITY REGIME" field with independent header and color. Previously the regime was inline text after VRP. Regime computation unchanged (rule 106).
